...
export default function SkillSelect(props: SelectProps) {
  ...
  const deleteHandler = (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    ...
    setChosenSkills(chosenSkills.filter((skill) => skill.value !== value));
    setFilteredOptions(
      props.options?.filter((option) => option.value !== value)
    );
  };
